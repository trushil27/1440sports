...
**GRID FIT coverage (13–15 Sep 2026):** GRID FIT is generated by code from `seeds/sponsor_categories.json`
(single-word lane tokens vs the brief's `industry_meta`); a brand on the sponsor table with no category row
is invisible to it, so the panel showed Red Bull "OPEN" to a cybersecurity brief while 1Password held the
lane (N° 154), and Mercedes/Red Bull/Ferrari "OPEN" to a prediction market while Nasdaq, Gate.io and BingX
sat on them (N° 252). Rows added for the cybersecurity, payments, betting/casino/lottery, crypto-exchange
and broker lanes (78 of 346 partner rows categorised); CROWDED cannot fire today because no caller passes
`industry_tokens_extra`. `session_case check` does not render — a page-1 overflow only shows at `build`.
**N° 252 Polymarket → Cadillac F1 Team (71/100, MODE B, 15 Sep 2026, on request):** the engine's OF gate
(brand fit < 12 ⇒ ops fit excluded from ranking) ranks it 62; built with `"threshold": 60` and the gate
stated in the bottom line rather than moving brand fit to pass. Buyer = Ari Borod (President of Sports
Business Development); Walter (TWG) DOJ/SEC investigation and the 1789 Capital / House Oversight facts are
risk one. **Team concentration (MD, 15 Sep):** 17 of 26 shown FE cases point at Andretti (10) or Jaguar (7);
